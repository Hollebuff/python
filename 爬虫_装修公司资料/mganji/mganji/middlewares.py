...
class MyproxiesSpiderMiddleware(object):
    ...
    def process_request(self, request, spider):
        ...
        # 初始化的代理iP开始是没有加协议的
        thisip = random_ip(proxys[-50:])
        while thisip in self.delete:
            print('已失效代理IP-------------------------------------------------aaa', thisip)
            thisip = random_ip(proxys[-5:])
            if thisip not in self.delete:
                print('失效重取正常代理IP-----------------------------bbb', thisip)
                request.meta["proxy"] = "https://" + thisip
                break
        else:
            print("初始请求ip this is ip:" + thisip)
            request.meta["proxy"] = "https://" + thisip
    ...
    def process_response(self, request, response, spider):
        ...
        if response.status != 200:
            reader_path = 'E:/爬虫_装修公司资料/mganji/ip.txt'
            proxys = reader_ip(reader_path)
            rsp_deleip = request.meta['proxy'].replace('https://', '')
            self.delete.add(rsp_deleip)
            thisip = random_ip(proxys[-5:])
            print('被拒绝访问原因------------------------', response.status, '代理ip已添至黑名单', rsp_deleip)
            while thisip in self.delete:
                print('失效代理IP------------------------------age', thisip)
                thisip = random_ip(proxys[-5:])
                if thisip not in self.delete:
                    print('正常代理IP-----------age', thisip)
                    request.meta["proxy"] = "https://" + thisip
                    break
            else:
                print('--'*50)
                logger.warning("状态码 no 200，重新请求 this is response ip:" + thisip)
                # 对当前request加上代理
                request.meta['proxy'] = "https//" + thisip
                return request
        return response
    ...
    def process_exception(self, request, exception, spider):
        ...
        if isinstance(exception, self.EXCEPTIONS_TO_CHANGE) and request.meta.get('proxy', False):
            delete_url = request.meta['proxy'].replace('https://', '')
            self.delete.add(delete_url)
            print('++'*75)
            logger.debug("该代理IP  {} 失效已添加到失效set集合，原因：{}.".format(request.meta['proxy'], exception))
            return request.replace(dont_filter=True)


class MganjiSpiderMiddleware(object):
...
class MganjiDownloaderMiddleware(object):
    # Not all methods need to be defined. If a method is not defined,
